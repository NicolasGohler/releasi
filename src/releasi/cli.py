...
async def _get_repo():
    ...
    from releasi.db.engine import init_db, get_session_factory
...
async def _cleanup(session):
    ...
    await session.close()
    from releasi.db.engine import close_db
    await close_db()
...
def account_list():
    ...
    async def _list():
        ...
        if not accounts:
            console.print("[dim]No accounts found. Use 'releasi account add' to add one.[/dim]")
            return
...
def campaign_create(
    ...
    async def _create():
        ...
        if connect_msg:
            from releasi.campaign.template import extract_variables
...
def campaign_import(
    ...
    async def _import():
        ...
        from releasi.db.models import LeadStatus, CampaignLeadAssignment
        from sqlalchemy import select
        from releasi.db.models import Lead
...
def campaign_set_template(
    ...
    async def _set():
        ...
        if connect_msg:
            from releasi.campaign.template import validate_template
...
def local_login(
    ...
    async def _local_login():
        from releasi.linkedin.local_login import LocalLoginSession
...
def run():
    ...
    console.print("[bold]Starting releasi scheduler...[/bold]")
    console.print("Press Ctrl+C to stop.\n")

    async def _run_scheduler():
        from releasi.scheduler.runner import start_scheduler
...
def check_acceptances_cmd(
    ...
    async def _check():
        ...
        from releasi.db.models import LeadStatus, ActionType, ActionLogStatus
        from releasi.linkedin.browser import LinkedInBrowser
        ...
        try:
            ...
            console.print("\n[bold]Step 1: loading connections page...[/bold]")
            conn_page = await context.new_page()
            try:
                conn_actions = LinkedInActions(conn_page)
                conn_result = await conn_actions.get_recent_connections(cutoff_hours=CUTOFF_HOURS)
            finally:
            ...
            recent_slugs = set(conn_result.slugs)
            console.print(f"  Recent connections found: {len(recent_slugs)} (cutoff {CUTOFF_HOURS}h, hit_cutoff={conn_result.hit_cutoff})")
